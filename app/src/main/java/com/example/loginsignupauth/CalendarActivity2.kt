        ...
        databaseReference.child(date).addListenerForSingleValueEvent(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                if (event != null) {
                    ...
                    Toast.makeText(this@CalendarActivity2, "Event: $event", Toast.LENGTH_SHORT).show()
                } else {
                    editTextText2.setText("")
                    Toast.makeText(this@CalendarActivity2, "No event found!", Toast.LENGTH_SHORT).show()
                }
            }

            override fun onCancelled(error: DatabaseError) {
                Toast.makeText(this@CalendarActivity2, "Error fetching event!", Toast.LENGTH_SHORT).show()
            }
        })
    }
